...
// ====================================================================
// 3. SETUP 函式：初始化畫布、選單和 Iframe 【重要修改】
// ====================================================================
...
function setup() {
    ...
    // 將 position 改為 absolute，讓漢堡圖示可以固定在畫面上
    ...
    hamburgerButton.style('position', 'fixed'); // 保持固定在畫面左上角
    ...
    hamburgerButton.style('background-color', 'rgba(26, 83, 192, 0.8)'); // 藍色
    ...
    hamburgerButton.style('z-index', '1001'); // 確保在菜單之上
    hamburgerButton.style('display', 'none'); // 預設隱藏 (只在小螢幕顯示)
...
function handleMenuClick(action, url) {
    ...
    } else if (action === 'work' || action === 'notes' || action === 'quizNotes' || action === 'quiz') {
        iframe.src = action === 'quiz' ? 'https://shing0411.github.io/20251106-1/' : url;
        
    } else if (action === 'tku') {
        iframe.src = url;
        
    } else if (action === 'ettku') {
        iframe.src = url;
    }
